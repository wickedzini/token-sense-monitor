...
export class ABTestService {
  ...
  private simulateTest(currentModel: string, candidateModel: string, sampleSize: number): ABTestResults {
    ...
    const current = Object.keys(modelDiffMatrix).find(m => 
      currentModel.toLowerCase().includes(m.toLowerCase())
    );
    
    if (current && modelDiffMatrix[current]) {
      const candidate = Object.keys(modelDiffMatrix[current]).find(m => 
        candidateModel.toLowerCase().includes(m.toLowerCase())
      );
      
      if (candidate) {
        qualityDeltaRange = modelDiffMatrix[current][candidate].qualityDeltaRange;
        latencyDeltaRange = modelDiffMatrix[current][candidate].latencyDeltaRange;
      }
    }
